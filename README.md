...
### Objective function
<img src="http://chart.googleapis.com/chart?cht=tx&chl= min   " style="border:none;"><img src="https://render.githubusercontent.com/render/math?math=\quad
\underbrace{\sum_{t\in T}\sum_{i\in I}\sum_{f\in F} (M_i + G_{if}) x_{tif}  +\sum_{t\in T}\sum_{f\in F}  K_{f} w_{tf}}_{訂貨成本}\\+ \sum_{t \in T}\sum_{i\in I} (\underbrace{C_{i}^o y_{ti}}_{存貨成本} +\underbrace{\beta_i C_{i}^b z_{ti} + (1 - \beta_i)C_{i}^l z_{ti}}_{缺貨成本})">
